...
public class MongoDBConnection implements DBConnection {
	 ...
	 * 
	 * 
	 * db.users.update( { user_id:"1111" }, { $pushall:{ visited:["aaaa","bbbb"]
	 * } } )
	 * 
	 */
	...
	public JSONArray recommendRestaurants(String userId) {
		// TODO Auto-generated method stub
		// Step 1, fetch all the restaurants (id) this user has visited (which
		// API, which table? history)
		// Step 2, given all these restaurants, what are the categories (which
		// table? restaurants)
		// Step 3, given these categories, what are the restaurants that belong
		// to them (which table?restaurants)
		// Step 4, filter restaurants that this user has visited (which
		// table?history)
		...
		try {
			...
			for (String businessId : allRestaurants) {
				...
				if (!visitedRestaurants.contains(businessId)) {
					...
					count++;
					if (count >= MAX_RECOMMENDED_RESTAURANTS) {// MAX_RECOMMENDED_RESTAURANTS
																// == 10 we put
																// it.
						break;
					}
	...
	public Set<String> getBusinessId(String category) {
		...
		FindIterable<Document> iterable = db.getCollection("restaurants").find(regex("categories", category));//regular expression match :   means that if "Japanese" is a subset of "Chinaese, Japanese, Korean".
	...
	public JSONArray searchRestaurants(String userId, double lat, double lon, String term) {
		...
		try {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				// Question: why using upsert instead of insert directly? update
				// or insert? if i can find, then update, or just insert.
